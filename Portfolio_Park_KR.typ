  ...
  #grid(
    ...
    [
      ...
      1. *Large Language Model* \
        - *KRAFT³-QA* #pill[Publications\#3] : 금융 텍스트-표 혼합 질의응답을 체계적으로 평가할 벤치마크가 부재한 문제를 해결하기 위해, 공시 데이터 기반 QA 데이터셋과 Tool-augmented Agent 평가 프로토콜을 설계·구현하고 대규모 LLM 실험을 수행함
      ...
      2. *Time Series Forecasting*
        ...
        - *TSFM Catastrophic Forgetting* #pill[Projects\#12] : 자산 유니버스가 순차적으로 확장되는 금융 환경에서 시계열 모델의 망각 현상이 체계적으로 분석되지 않은 문제를 해결하기 위해, 시계열 파운데이션 모델의 순차 학습 프로토콜과 평가 프레임워크를 설계하고 실증 분석을 수행함
      연세대학교 정보대학원 계약학과 AI핀테크 과정의 "재무와 딥러닝" 과목의 강의조교로 활동, 전체 수업의 약 1/3에 해당하는 실습 강의을 담당
    ],

    [*성과)*], [국내 저널(KCI) 1편 게재, 국내 학회 1편 발표, SCIE Q1 저널 1편 심사 중, SCIE/SSCI 저널 각각 1편 추가 제출 준비 중],
...
#block(spacing: 1em)[
  ...
  #grid(
    ...
    [
      ...
      이에 따라 변형 텍스트 정규화, 하이브리드 분류 구조, 자체 모델 업데이트 기능을 갖춘 #accent[자기 개선형 스팸 탐지 시스템]을 설계·구현함 (3인 공동창업)
    ],
...
#block(spacing: 1em)[
  ...
  #grid(
    ...
    [
      ...
      특히, Tool-augmented Agent의 성능을 체계적으로 평가할 수 있는 금융 특화 벤치마크가 부재
    ],
...
#block(spacing: 1em)[
  ...
  #grid(
    ...
    [
      ...
      학교 단위 플랫폼 확장 모델과 강의 게시판 구독, 제휴 및 광고 수익을 포함한 서비스 확장 가능성을 제시하여, 학술 중심 커뮤니티의 지속 가능성도 검토하는 형태의 제품 구조를 발표함
    ],
